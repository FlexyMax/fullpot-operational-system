...
- **Accordion rows nested inside `<table>`s (e.g. Prebook to Invoice's Date → Customer → Lines → detail-tabs drill-down):** when a nested panel needs a fixed total height split between a header strip and a content area, **don't use `flex flex-col` + `flex-1` to distribute the remaining height** — give the content area its own explicit pixel height instead (e.g. header `h-10` + content `h-[190px]` to total `230px`, as two plain stacked blocks, no `flex` on the parent at all). `flex-1`'s height computation stopped resolving once this content was three `<table>`s deep (header rendered, the tab content area rendered at 0 height) even though the exact same flex pattern worked fine as a top-level page panel. Stick to explicit `h-[Npx]` at every level when nesting inside tables — don't assume flex-grow distribution survives table nesting.
