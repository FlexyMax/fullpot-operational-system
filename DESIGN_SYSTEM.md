...
- **Accordion rows nested inside `<table>`s (e.g. Prebook to Invoice's Date → Customer → Lines → detail-tabs drill-down):** `flex flex-col` + `flex-1` to distribute height between a header strip and a scrollable content area **does not reliably resolve three or more `<table>`s deep** — even giving the `flex-1` ancestor a definite pixel height didn't fix it; the content area still rendered at 0 height while the header next to it rendered fine. The exact same flex pattern works correctly as a top-level page panel, just not this deep inside nested tables.
  Use `max-height` + `overflow-y-auto` instead, at every level — it's what the two outer nesting levels (Customers-under-Date, Lines-under-Customer) used from the start and they never had this problem. Concretely: drop `flex flex-col h-full` from a nested panel's root and `flex-1 overflow-auto` from its scrollable child; replace with a plain `<div>` root (no flex, sized by content) and `max-h-[Npx] overflow-y-auto` directly on the table wrapper. Don't reach for flex-grow distribution to split header/content height inside a deeply-nested table — it doesn't survive that nesting in this codebase, `max-height` does.
