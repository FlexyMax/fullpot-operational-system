...
- **Accordion rows nested inside `<table>`s (e.g. Prebook to Invoice's Date → Customer drill-down):** `flex flex-col` + `flex-1` to distribute height between a header strip and a scrollable content area **does not reliably resolve three or more `<table>`s deep** — even giving the `flex-1` ancestor a definite pixel height didn't fix it; the content area rendered at 0 height while the header next to it rendered fine. The exact same flex pattern works correctly as a top-level page panel, just not this deep inside nested tables.
  Use `max-height` + `overflow-y-auto` instead, at any level you do nest — it's what Customers-under-Date uses and it never had this problem: drop `flex flex-col h-full` from a nested panel's root and `flex-1 overflow-auto` from its scrollable child; replace with a plain `<div>` root (no flex, sized by content) and `max-h-[Npx] overflow-y-auto` directly on the table wrapper.
  **Also reconsider whether the content needs to be nested in a `<table>` row at all — most of the time it shouldn't be.** Prebook to Invoice went through three iterations: first the Lines grid AND the detail-tabs panel were both nested inside Customer rows (Date → Customer → Lines → Tabs, 3 levels); then Lines stayed nested but the detail-tabs panel was pulled out to a standalone panel below; the final shape pulls Lines out too, so only **one** level of table-row nesting remains (Date → Customer), and both "Closed Prebook box by date and customer" (Lines) and the detail tabs are standalone panels stacked below the Date Picker, each one reading off the same selection state (`selectedDate`/`selectedCustUq` filter the Lines query, `selectedUnico` drives the detail tabs) instead of being rendered inside a table cell. A `<table>` row is the wrong host for anything beyond a small, single-purpose sub-grid — a full grid-with-its-own-toolbar or a multi-tab sub-page reads much better as a sibling panel than as nested `<tr><td>` content. Default to a standalone panel driven by shared selection state; only nest in a table row for something genuinely small and grid-shaped.
