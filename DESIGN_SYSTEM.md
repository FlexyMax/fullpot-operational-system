...
- **Accordion rows nested inside `<table>`s (e.g. Prebook to Invoice's Date → Customer → Lines drill-down):** `flex flex-col` + `flex-1` to distribute height between a header strip and a scrollable content area **does not reliably resolve three or more `<table>`s deep** — even giving the `flex-1` ancestor a definite pixel height didn't fix it; the content area rendered at 0 height while the header next to it rendered fine. The exact same flex pattern works correctly as a top-level page panel, just not this deep inside nested tables.
  Use `max-height` + `overflow-y-auto` instead, at every level you do nest — it's what Customers-under-Date and Lines-under-Customer use and they never had this problem: drop `flex flex-col h-full` from a nested panel's root and `flex-1 overflow-auto` from its scrollable child; replace with a plain `<div>` root (no flex, sized by content) and `max-h-[Npx] overflow-y-auto` directly on the table wrapper.
  **But also reconsider whether the content needs to be nested in a `<table>` row at all.** Prebook to Invoice's detail-tabs panel (Invoiced Prebooks/Assigned Stock/Purchase/Stock OM/Similar Products) was tried nested under each Lines row and never looked right even after the height issue was fixed — too cramped, and a `<table>` row is the wrong host for a multi-tab sub-page. It now lives as its own standalone panel below the Lines grid (driven by the same `selectedUnico` state, just rendered outside any table), which is both simpler and reads better. Two levels of nesting (Date → Customer → Lines) is the practical ceiling here — don't push a third level into a table row by default; ask whether a sibling panel reading off the same selection state fits better first.
