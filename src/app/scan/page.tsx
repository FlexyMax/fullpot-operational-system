"use client";

import { useState, useRef, useEffect, useCallback } from "react";
import { useSession }       from "next-auth/react";
import { useRouter }        from "next/navigation";
import { useQuery, useQueryClient } from "@tanstack/react-query";
import {
    ArrowLeft, RefreshCcw, Loader2, Scan,
    Check, X, Trash2, AlertTriangle,
} from "lucide-react";
import { cn }    from "@/lib/utils";
import { toast } from "sonner";
import { usePagePermissions } from "@/lib/permissions";

// ─── Helpers ──────────────────────────────────────────────────────────────────
const t      = (v: any) => String(v ?? "").trim();
const fmtN   = (v: any) => { const n = Number(v ?? 0); return isNaN(n) ? "—" : n.toLocaleString("en-US"); };
const fmt2   = (v: any) => parseFloat(v ?? 0).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
const fmtDate = (v: any) => {
    if (!v) return "";
    const s = t(v);
    const iso = s.match(/^(\d{4})-(\d{2})-(\d{2})/);
    if (iso) return new Date(+iso[1], +iso[2]-1, +iso[3]).toLocaleDateString("en-US");
    const d = new Date(s); return isNaN(d.getTime()) ? s : d.toLocaleDateString("en-US");
};

function Th({ children, className }: { children: any; className?: string }) {
    return <th className={cn("px-2 py-1.5 text-left font-bold whitespace-nowrap text-gray-600 border-b border-gray-200 bg-gray-50 sticky top-0 z-10 text-[11px]", className)}>{children}</th>;
}
function Td({ children, className }: { children: any; className?: string }) {
    return <td className={cn("px-2 py-1 whitespace-nowrap text-[11px] border-l border-gray-100 first:border-l-0", className)}>{children}</td>;
}
function StatBox({ label, value, sub, color = "gray" }: { label: string; value: any; sub?: string; color?: string }) {
    return (
        <div className={cn(
            "flex flex-col items-center justify-center rounded-lg px-3 py-2 min-w-[90px]",
            color === "orange" && "bg-[#FB7506]/10 border border-[#FB7506]/30",
            color === "green"  && "bg-green-50 border border-green-200",
            color === "blue"   && "bg-blue-50 border border-blue-200",
            color === "gray"   && "bg-gray-100 border border-gray-200",
            color === "red"    && "bg-red-50 border border-red-200",
        )}>
            <span className={cn(
                "text-[18px] font-black tabular-nums",
                color === "orange" && "text-[#FB7506]",
                color === "green"  && "text-green-700",
                color === "blue"   && "text-blue-700",
                color === "gray"   && "text-gray-700",
                color === "red"    && "text-red-600",
            )}>{fmtN(value)}</span>
            <span className="text-[9px] font-bold text-gray-500 uppercase tracking-wide text-center">{label}</span>
            {sub && <span className="text-[8px] text-gray-400 text-center">{sub}</span>}
        </div>
    );
}

// ─── Tab definitions ──────────────────────────────────────────────────────────
const TABS = [
    { id: "pending",   label: "Pending Scan",   color: "text-red-600",   view: "pending" },
    { id: "in-transit",label: "In Transit",     color: "text-blue-600",  view: "in-transit" },
    { id: "scanned-equal", label: "Scan = Physical", color: "text-green-600", view: "scanned-equal" },
    { id: "scanned-boxes", label: "Scanned Boxes", color: "text-gray-700",   view: "scanned-boxes" },
    { id: "sys-not-phy",   label: "Sys ≠ Phy",  color: "text-purple-600",view: "system-not-physical" },
    { id: "sys-less",      label: "Sys > Phy",  color: "text-orange-600",view: "system-less-physical" },
    { id: "sys-eq",        label: "Sys = Phy",  color: "text-teal-600",  view: "system-equal-physical" },
] as const;

type TabId = typeof TABS[number]["id"];

const PAGE_SIZE = 50;

// ─── Main Page ────────────────────────────────────────────────────────────────
export default function PhysicalScanPage() {
    const { data: session, status } = useSession();
    const router = useRouter();
    const { canEdit, canDelete } = usePagePermissions("scan");
    const qc = useQueryClient();

    const [activeTab,    setActiveTab]   = useState<TabId>("pending");
    const [barcode,      setBarcode]     = useState("");
    const [rack,         setRack]        = useState("");
    const [scanning,     setScanning]    = useState(false);
    const [lastScan,     setLastScan]    = useState<{ ok: boolean; msg: string } | null>(null);
    const [pendingPage,  setPendingPage] = useState(1);
    const [pendingRows,  setPendingRows] = useState<any[]>([]);
    const [pendingTotal, setPendingTotal]= useState(0);
    const [pendingLoading, setPendingLoading] = useState(false);
    const [hasMore,      setHasMore]    = useState(true);
    const [viewKey,      setViewKey]    = useState(0);

    const barcodeRef  = useRef<HTMLInputElement>(null);
    const sentinelRef = useRef<HTMLDivElement>(null);

    // Auto-focus barcode on mount
    useEffect(() => { barcodeRef.current?.focus(); }, []);

    // ── Totals ────────────────────────────────────────────────────────────────
    const { data: totals, refetch: refetchTotals } = useQuery({
        queryKey: ["scan-totals", viewKey],
        queryFn: async () => {
            const r = await fetch("/api/physical-scan/totals");
            const j = await r.json();
            return j;
        },
        refetchInterval: 30000,
    });

    // ── Pending (paginated) ───────────────────────────────────────────────────
    const loadPending = useCallback(async (page: number, reset: boolean) => {
        setPendingLoading(true);
        try {
            const r = await fetch(`/api/physical-scan/ready-to-scan?page=${page}&size=${PAGE_SIZE}`);
            const j = await r.json();
            const rows: any[] = j.rows ?? [];
            const total = j.total ?? 0;
            setPendingTotal(total);
            setPendingRows(prev => reset ? rows : [...prev, ...rows]);
            setHasMore(rows.length === PAGE_SIZE);
            setPendingPage(page);
        } catch { /* ignore */ }
        finally { setPendingLoading(false); }
    }, []);

    useEffect(() => { loadPending(1, true); }, [loadPending, viewKey]);

    // Infinite scroll for pending tab
    useEffect(() => {
        if (activeTab !== "pending") return;
        const el = sentinelRef.current;
        if (!el) return;
        const obs = new IntersectionObserver(entries => {
            if (entries[0].isIntersecting && !pendingLoading && hasMore) {
                loadPending(pendingPage + 1, false);
            }
        }, { threshold: 0.1 });
        obs.observe(el);
        return () => obs.disconnect();
    }, [activeTab, pendingLoading, hasMore, pendingPage, loadPending]);

    // ── Other views ───────────────────────────────────────────────────────────
    const activeTabDef = TABS.find(t => t.id === activeTab)!;
    const viewSPKey    = activeTabDef.view;

    const { data: viewRows = [], isFetching: loadingView } = useQuery({
        queryKey: ["scan-view", viewSPKey, viewKey],
        enabled: activeTab !== "pending",
        queryFn: async () => {
            const r = await fetch(`/api/physical-scan/views?v=${viewSPKey}`);
            const j = await r.json();
            return Array.isArray(j) ? j : [];
        },
    });

    // ── Scan action ──────────────────────────────────────────────────────────
    const handleScan = async () => {
        const code = barcode.trim();
        if (!code) { barcodeRef.current?.focus(); return; }
        setScanning(true);
        setLastScan(null);
        try {
            const r = await fetch("/api/physical-scan/scan", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ compuesto: code, rack: rack.trim() }),
            });
            const j = await r.json();
            if (!r.ok || !j.success) {
                const msg = j.error || "Scan failed";
                setLastScan({ ok: false, msg });
                toast.error(msg);
            } else {
                setLastScan({ ok: true, msg: `✓ ${code} scanned` });
                toast.success(`Scanned: ${code}`);
                setBarcode("");
                setViewKey(k => k + 1);
            }
        } catch (e: any) {
            setLastScan({ ok: false, msg: e.message });
            toast.error(e.message);
        } finally {
            setScanning(false);
            barcodeRef.current?.focus();
        }
    };

    const handleDeleteAll = () => {
        toast("Delete ALL scanned records? This cannot be undone.", {
            duration: 10000,
            action: { label: "Delete All", onClick: async () => {
                try {
                    const r = await fetch("/api/physical-scan/scan", { method: "DELETE" });
                    const j = await r.json();
                    if (!r.ok || !j.success) throw new Error(j.error || "Failed");
                    toast.success("All scanned records deleted");
                    setViewKey(k => k + 1);
                } catch (e: any) { toast.error(e.message); }
            }},
            cancel: { label: "Cancel", onClick: () => {} },
        });
    };

    if (status === "loading") return null;
    if (status === "unauthenticated") { router.push("/login"); return null; }

    const rows = activeTab === "pending" ? pendingRows : viewRows;

    return (
        <div className="flex flex-col h-screen bg-[#f4f6f8] overflow-hidden font-sans text-[#333]">

            {/* ── Top bar ──────────────────────────────────────────────────── */}
            <div className="h-12 bg-[#374151] flex items-center justify-between px-4 shrink-0 text-white">
                <div className="flex items-center gap-3">
                    <button onClick={() => router.push("/menu")} className="hover:bg-white/10 p-1.5 rounded">
                        <ArrowLeft size={18} />
                    </button>
                    <span className="font-black text-xs uppercase tracking-widest text-[#FB7506]">FOS</span>
                    <div className="w-px h-4 bg-white/20" />
                    <Scan size={14} className="text-[#FB7506]" />
                    <span className="font-bold text-xs uppercase tracking-tight">Physical Inventory Scan</span>
                </div>
                <div className="hidden sm:flex items-center gap-3 text-[10px] font-bold uppercase">
                    <span className="text-gray-400">User: <span className="text-white">{session?.user?.name || "OPERATOR"}</span></span>
                    <span className="text-green-500">● Online</span>
                </div>
            </div>

            {/* ── Stats bar ────────────────────────────────────────────────── */}
            <div className="bg-white border-b border-gray-200 px-3 py-2 shrink-0 overflow-x-auto">
                <div className="flex items-center gap-2 min-w-max">
                    <StatBox label="System Stock" value={totals?.Total_stock}       color="gray"   />
                    <StatBox label="Qty Scanned"  value={totals?.Total_stock_QPI}   color="green"  />
                    <StatBox label="To Scan"      value={totals?.Total_Stock_TO}    color="red"    />
                    <div className="w-px h-10 bg-gray-200 mx-1" />
                    <StatBox label="Ready Boxes"  value={totals?.Total_ready}       color="gray"   />
                    <StatBox label="Box Scanned"  value={totals?.Total_QPI}         color="blue"   />
                    <StatBox label="Box To Read"  value={totals?.Total_To_read}     color="orange" />
                    <div className="w-px h-10 bg-gray-200 mx-1" />
                    <StatBox label="In Transit"   value={totals?.total_flight}      color="gray"   sub="pcs" />
                    <StatBox label="Scanned ND"   value={totals?.Total_QPI_ND}      color="blue"   sub="next day" />
                    <StatBox label="To Read ND"   value={totals?.Total_To_ND}       color="orange" sub="next day" />
                    <div className="ml-2">
                        <button onClick={() => setViewKey(k => k+1)}
                            className="flex items-center gap-1 px-2 py-1 text-[10px] font-black text-gray-500 hover:text-gray-700 bg-gray-100 hover:bg-gray-200 rounded border border-gray-200">
                            <RefreshCcw size={10} /> Refresh
                        </button>
                    </div>
                </div>
            </div>

            {/* ── Scan input area ──────────────────────────────────────────── */}
            <div className="bg-white border-b border-gray-200 px-3 py-2 shrink-0">
                <div className="flex flex-wrap items-center gap-2">
                    {/* Barcode */}
                    <div className="flex items-center gap-1 flex-1 min-w-[200px] max-w-xs">
                        <label className="text-[10px] font-black text-gray-500 uppercase tracking-wide shrink-0">Barcode</label>
                        <input
                            ref={barcodeRef}
                            value={barcode}
                            onChange={e => setBarcode(e.target.value)}
                            onKeyDown={e => { if (e.key === "Enter") handleScan(); }}
                            placeholder="Scan or type barcode..."
                            className="flex-1 border border-gray-300 rounded px-2 py-1.5 text-[12px] font-mono font-bold focus:outline-none focus:ring-2 focus:ring-[#FB7506] uppercase"
                            autoComplete="off"
                        />
                    </div>

                    {/* Rack */}
                    <div className="flex items-center gap-1">
                        <label className="text-[10px] font-black text-gray-500 uppercase tracking-wide shrink-0">Rack</label>
                        <input
                            value={rack}
                            onChange={e => setRack(e.target.value)}
                            onKeyDown={e => { if (e.key === "Enter") handleScan(); }}
                            placeholder="Rack (opt)"
                            className="w-28 border border-gray-300 rounded px-2 py-1.5 text-[12px] font-mono focus:outline-none focus:ring-2 focus:ring-[#FB7506] uppercase"
                        />
                    </div>

                    {/* Scan button */}
                    <button onClick={handleScan} disabled={scanning || !barcode.trim() || !canEdit}
                        className="flex items-center gap-2 px-4 py-1.5 text-[11px] font-black text-white bg-green-600 hover:bg-green-500 rounded disabled:opacity-40 transition-all">
                        {scanning ? <Loader2 size={12} className="animate-spin" /> : <Scan size={12} />}
                        SCAN
                    </button>

                    {/* Last scan result */}
                    {lastScan && (
                        <div className={cn(
                            "flex items-center gap-1.5 px-3 py-1 rounded text-[11px] font-bold",
                            lastScan.ok ? "bg-green-100 text-green-700 border border-green-200" : "bg-red-100 text-red-700 border border-red-200"
                        )}>
                            {lastScan.ok ? <Check size={12} /> : <X size={12} />}
                            {lastScan.msg}
                        </div>
                    )}

                    {/* Delete All */}
                    {canDelete && (
                        <button onClick={handleDeleteAll}
                            className="ml-auto flex items-center gap-1 px-3 py-1.5 text-[10px] font-black text-white bg-red-600 hover:bg-red-500 rounded transition-all">
                            <Trash2 size={10} /> Delete All Scanned
                        </button>
                    )}
                </div>
            </div>

            {/* ── Tabs ────────────────────────────────────────────────────── */}
            <div className="bg-white border-b border-gray-200 px-3 flex items-center gap-0 overflow-x-auto shrink-0">
                {TABS.map(tab => (
                    <button key={tab.id}
                        onClick={() => setActiveTab(tab.id)}
                        className={cn(
                            "px-3 py-2 text-[10px] font-black uppercase tracking-widest whitespace-nowrap border-b-2 transition-all",
                            activeTab === tab.id
                                ? `border-[#FB7506] ${tab.color}`
                                : "border-transparent text-gray-500 hover:text-gray-700"
                        )}
                    >{tab.label}</button>
                ))}
                {(activeTab !== "pending" && loadingView) && (
                    <Loader2 size={12} className="animate-spin text-gray-400 ml-2 shrink-0" />
                )}
                <span className="ml-auto text-[10px] text-gray-400 font-bold px-2 shrink-0">
                    {activeTab === "pending"
                        ? `${pendingRows.length} / ${pendingTotal}`
                        : `${viewRows.length} rows`}
                </span>
            </div>

            {/* ── Table ───────────────────────────────────────────────────── */}
            <div className="flex-1 overflow-hidden px-2 pb-2 pt-2 min-h-0">
                <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden h-full flex flex-col">
                    <div className="flex-1 overflow-auto min-h-0">
                        {activeTab === "pending"  && <PendingTable rows={pendingRows} loading={pendingLoading && pendingPage === 1} sentinelRef={sentinelRef} hasMore={hasMore} loadingMore={pendingLoading && pendingPage > 1} />}
                        {activeTab === "in-transit"   && <InTransitTable rows={viewRows} />}
                        {activeTab === "scanned-equal" && <AwbTable rows={viewRows} title="Scanned = Physical" />}
                        {activeTab === "scanned-boxes" && <ScannedBoxesTable rows={viewRows} />}
                        {activeTab === "sys-not-phy"   && <SysNotPhyTable rows={viewRows} />}
                        {activeTab === "sys-less"      && <SysLessPhyTable rows={viewRows} />}
                        {activeTab === "sys-eq"        && <SysLessPhyTable rows={viewRows} />}
                    </div>
                </div>
            </div>
        </div>
    );
}

// ─── Table sub-components ─────────────────────────────────────────────────────
function PendingTable({ rows, loading, sentinelRef, hasMore, loadingMore }: any) {
    return (
        <table className="min-w-full text-left">
            <thead>
                <tr>
                    <Th>Barcode</Th>
                    <Th>Farm</Th>
                    <Th>AWB</Th>
                    <Th className="text-right">Lot</Th>
                    <Th className="text-right">Box Qty</Th>
                    <Th className="text-right">Sold</Th>
                    <Th className="text-right">Stock Sys</Th>
                    <Th className="text-right">Open Inv</Th>
                    <Th className="text-right">Available</Th>
                    <Th className="text-right">Scanned</Th>
                    <Th className={cn("text-right")}>Diff</Th>
                    <Th>Ship Date</Th>
                    <Th>Avail Date</Th>
                    <Th>Product</Th>
                </tr>
            </thead>
            <tbody>
                {loading && <tr><td colSpan={14} className="p-8 text-center text-gray-400 italic"><Loader2 size={14} className="animate-spin inline mr-2" />Loading...</td></tr>}
                {!loading && rows.map((r: any, i: number) => {
                    const diff = Number(r.Diff ?? r.diff ?? 0);
                    return (
                        <tr key={i} className="border-b odd:bg-white even:bg-gray-50 hover:bg-blue-50 text-gray-700">
                            <Td className="font-mono font-bold">{t(r.barcode)}</Td>
                            <Td className="font-bold text-[#FB7506]">{t(r.farm)}</Td>
                            <Td className="font-mono text-blue-700">{t(r.awbcode)}</Td>
                            <Td className="text-right">{fmtN(r.lote)}</Td>
                            <Td className="text-right">{fmtN(r.box_qty)}</Td>
                            <Td className="text-right">{fmtN(r.qty_sale)}</Td>
                            <Td className="text-right">{fmtN(r.stock)}</Td>
                            <Td className="text-right">{fmtN(r.OBox)}</Td>
                            <Td className="text-right">{fmtN(r.available)}</Td>
                            <Td className="text-right font-bold text-green-700">{fmtN(r.QPI)}</Td>
                            <Td className={cn("text-right font-black", diff < 0 ? "text-red-600" : diff > 0 ? "text-orange-600" : "text-green-600")}>{diff}</Td>
                            <Td>{t(r.Grower_Ship_date)}</Td>
                            <Td>{t(r.Avail_date)}</Td>
                            <Td className="max-w-[200px] truncate">{t(r.description)}</Td>
                        </tr>
                    );
                })}
                {!loading && rows.length === 0 && (
                    <tr><td colSpan={14} className="p-10 text-center text-gray-400 italic">No pending lots to scan</td></tr>
                )}
                <tr>
                    <td colSpan={14}>
                        <div ref={sentinelRef} className="flex items-center justify-center py-3">
                            {loadingMore && <Loader2 size={13} className="animate-spin text-gray-400" />}
                            {!loadingMore && !hasMore && rows.length > 0 && (
                                <span className="text-[10px] text-gray-400 italic">All records loaded</span>
                            )}
                        </div>
                    </td>
                </tr>
            </tbody>
        </table>
    );
}

function AwbTable({ rows, title }: { rows: any[]; title: string }) {
    return (
        <table className="min-w-full text-left">
            <thead>
                <tr>
                    <Th>Barcode</Th><Th>Farm</Th><Th>AWB</Th>
                    <Th className="text-right">Lot</Th>
                    <Th className="text-right">Box Qty</Th>
                    <Th className="text-right">Sold</Th>
                    <Th className="text-right">Stock Sys</Th>
                    <Th className="text-right">Open Inv</Th>
                    <Th className="text-right">Available</Th>
                    <Th className="text-right">Scanned</Th>
                    <Th>Ship Date</Th><Th>Avail Date</Th><Th>Product</Th>
                </tr>
            </thead>
            <tbody>
                {rows.map((r: any, i: number) => (
                    <tr key={i} className="border-b odd:bg-white even:bg-gray-50 hover:bg-blue-50 text-gray-700">
                        <Td className="font-mono font-bold">{t(r.barcode)}</Td>
                        <Td className="font-bold text-[#FB7506]">{t(r.farm)}</Td>
                        <Td className="font-mono text-blue-700">{t(r.awbcode)}</Td>
                        <Td className="text-right">{fmtN(r.lote)}</Td>
                        <Td className="text-right">{fmtN(r.box_qty)}</Td>
                        <Td className="text-right">{fmtN(r.qty_sale)}</Td>
                        <Td className="text-right">{fmtN(r.stock)}</Td>
                        <Td className="text-right">{fmtN(r.OBox)}</Td>
                        <Td className="text-right">{fmtN(r.available)}</Td>
                        <Td className="text-right font-bold text-green-700">{fmtN(r.QPI)}</Td>
                        <Td>{t(r.Grower_Ship_date)}</Td>
                        <Td>{t(r.Avail_date)}</Td>
                        <Td className="max-w-[200px] truncate">{t(r.description)}</Td>
                    </tr>
                ))}
                {rows.length === 0 && <tr><td colSpan={13} className="p-10 text-center text-gray-400 italic">No records</td></tr>}
            </tbody>
        </table>
    );
}

function InTransitTable({ rows }: { rows: any[] }) {
    return <AwbTable rows={rows} title="In Transit" />;
}

function ScannedBoxesTable({ rows }: { rows: any[] }) {
    return (
        <table className="min-w-full text-left">
            <thead>
                <tr>
                    <Th>AWB</Th><Th>AWB Date</Th>
                    <Th className="text-right">Lot</Th>
                    <Th className="text-right">Box #</Th>
                    <Th>ID</Th><Th>Rack</Th><Th>Grower</Th><Th>Product</Th>
                    <Th>Timestamp</Th>
                </tr>
            </thead>
            <tbody>
                {rows.map((r: any, i: number) => (
                    <tr key={i} className="border-b odd:bg-white even:bg-gray-50 hover:bg-blue-50 text-gray-700">
                        <Td className="font-mono font-bold text-blue-700">{t(r.awbcode)}</Td>
                        <Td>{fmtDate(r.date_invo)}</Td>
                        <Td className="text-right">{fmtN(r.lote)}</Td>
                        <Td className="text-right font-bold">{fmtN(r.box_no)}</Td>
                        <Td className="font-mono text-[10px]">{t(r.ID)}</Td>
                        <Td className="font-mono">{t(r.rack)}</Td>
                        <Td className="max-w-[120px] truncate">{t(r.grower)}</Td>
                        <Td className="max-w-[200px] truncate">{t(r.description)}</Td>
                        <Td className="text-gray-500">{r.timestamp ? new Date(r.timestamp).toLocaleString("en-US") : "—"}</Td>
                    </tr>
                ))}
                {rows.length === 0 && <tr><td colSpan={9} className="p-10 text-center text-gray-400 italic">No scanned boxes</td></tr>}
            </tbody>
        </table>
    );
}

function SysNotPhyTable({ rows }: { rows: any[] }) {
    return (
        <table className="min-w-full text-left">
            <thead>
                <tr>
                    <Th>Farm</Th><Th>AWB</Th>
                    <Th className="text-right">Lot</Th>
                    <Th className="text-right">Box Qty</Th>
                    <Th className="text-right">In Transit</Th>
                    <Th className="text-right">Sold</Th>
                    <Th className="text-right">Stock</Th>
                    <Th className="text-right">Un/Box</Th>
                    <Th>Customer</Th><Th>Case</Th><Th>Warehouse</Th><Th>Product</Th>
                </tr>
            </thead>
            <tbody>
                {rows.map((r: any, i: number) => (
                    <tr key={i} className="border-b odd:bg-white even:bg-gray-50 hover:bg-blue-50 text-gray-700">
                        <Td className="font-bold text-[#FB7506]">{t(r.farm)}</Td>
                        <Td className="font-mono text-blue-700">{t(r.awbcode)}</Td>
                        <Td className="text-right">{fmtN(r.lote)}</Td>
                        <Td className="text-right">{fmtN(r.box_qty)}</Td>
                        <Td className="text-right text-orange-600">{fmtN(r.qty_transit)}</Td>
                        <Td className="text-right">{fmtN(r.qty_sold)}</Td>
                        <Td className="text-right font-bold">{fmtN(r.stock)}</Td>
                        <Td className="text-right">{fmtN(r.tunits_x_box)}</Td>
                        <Td className="max-w-[90px] truncate">{t(r.customer)}</Td>
                        <Td>{t(r.case_sh)}</Td>
                        <Td className="max-w-[90px] truncate">{t(r.wp_name)}</Td>
                        <Td className="max-w-[200px] truncate">{t(r.description)}</Td>
                    </tr>
                ))}
                {rows.length === 0 && <tr><td colSpan={12} className="p-10 text-center text-gray-400 italic">No records</td></tr>}
            </tbody>
        </table>
    );
}

function SysLessPhyTable({ rows }: { rows: any[] }) {
    return (
        <table className="min-w-full text-left">
            <thead>
                <tr>
                    <Th>Barcode</Th><Th>Farm</Th><Th>AWB</Th>
                    <Th className="text-right">Lot</Th>
                    <Th>Box Date</Th>
                    <Th className="text-right">Box Qty</Th>
                    <Th className="text-right">Sold</Th>
                    <Th className="text-right">Stock</Th>
                    <Th className="text-right">Scanned</Th>
                    <Th className="text-right">Open Inv</Th>
                    <Th>Product</Th>
                </tr>
            </thead>
            <tbody>
                {rows.map((r: any, i: number) => (
                    <tr key={i} className="border-b odd:bg-white even:bg-gray-50 hover:bg-blue-50 text-gray-700">
                        <Td className="font-mono font-bold">{t(r.barcode)}</Td>
                        <Td className="font-bold text-[#FB7506]">{t(r.farm)}</Td>
                        <Td className="font-mono text-blue-700">{t(r.awbcode)}</Td>
                        <Td className="text-right">{fmtN(r.lote)}</Td>
                        <Td>{fmtDate(r.box_date)}</Td>
                        <Td className="text-right">{fmtN(r.box_qty)}</Td>
                        <Td className="text-right">{fmtN(r.qty_sale)}</Td>
                        <Td className="text-right font-bold">{fmtN(r.stock)}</Td>
                        <Td className="text-right font-bold text-green-700">{fmtN(r.QPI)}</Td>
                        <Td className="text-right">{fmtN(r.OBox)}</Td>
                        <Td className="max-w-[200px] truncate">{t(r.description)}</Td>
                    </tr>
                ))}
                {rows.length === 0 && <tr><td colSpan={11} className="p-10 text-center text-gray-400 italic">No records</td></tr>}
            </tbody>
        </table>
    );
}
