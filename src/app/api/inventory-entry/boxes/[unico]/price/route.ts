...
export async function PUT(req: NextRequest, { params }: P) {
    ...
    try {
        const r = await executeProcedure("sp_flower_inventory_stock_change_price", {
            unico:        unico,
            price_x_unit: num(b.price_x_unit),
            user_uq:      str(b.user_uq, 8),
        });
        const row = r.recordset?.[0];
        if (row?.error === 1 || row?.Error === 1) return NextResponse.json({ success: false, error: row.message || row.Message }, { status: 400 });
        serverAuditLog(PANTA, "Edit", "flower_packing_box", unico, "Change Price").catch(() => {});
