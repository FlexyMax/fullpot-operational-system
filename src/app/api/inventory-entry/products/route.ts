...
// SP params: @lnPageNumber int, @lnRowsOfPage int, @lcdescription varchar(1000)
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        const total = rows.length > 0 ? Number(rows[0].QueryTotalRecords ?? rows[0].TOTAL_RECORDS ?? rows[0].TOTAL ?? rows.length) : 0;
