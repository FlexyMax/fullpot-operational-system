...
export async function POST(req: NextRequest, { params }: { params: Promise<{ unico: string }> }) {
    try {
        ...
        if (b.insertAll) {
            await executeProcedure("sp_flower_invoice_automatic_charges_growers_insert_all", {
                lccharge_uq: txt(unico),
            });
            serverAuditLog(PANTA, "Insert", "flower_invoice_automatic_charges_growers", txt(unico)).catch(() => {});
            return NextResponse.json({ success: true });
        }

        const r = await executeProcedure("sp_flower_invoice_automatic_charges_growers_insert", {
            lccharge_uq: txt(unico),
            lcgrower_uq: txt(b.grower_uq),
        });
        const row = r.recordset?.[0];
        serverAuditLog(PANTA, "Insert", "flower_invoice_automatic_charges_growers", txt(row?.unico)).catch(() => {});
