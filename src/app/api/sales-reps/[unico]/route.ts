...
export async function PUT(req: NextRequest, { params }: P) {
    ...
    try {
        const r = await executeProcedure("sp_flower_salesmen_update", {
            lcunico:                unico,
        ...
        return NextResponse.json({ success: true, message: row?.message || row?.Message || "Sales rep updated." });
    } catch (err: any) {
...
export async function DELETE(_req: NextRequest, { params }: P) {
    const { unico } = await params;
    try {
        const r = await executeProcedure("sp_flower_salesmen_delete", { lcunico: unico });
        const row = r.recordset?.[0];
        if (row?.error === 1 || row?.Error === 1) {
            return NextResponse.json({ success: false, error: row.message || row.Message }, { status: 400 });
        }
        return NextResponse.json({ success: true, message: row?.message || row?.Message || "Sales rep deleted." });
    } catch (err: any) {
        return NextResponse.json({ success: false, error: err.message }, { status: 500 });
    }
}
