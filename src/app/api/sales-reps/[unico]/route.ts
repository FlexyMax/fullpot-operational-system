...
export async function GET(_req: NextRequest, { params }: P) {
    ...
    try {
        const r = await executeProcedure("sp_flower_salesman_uq", { lcunico: unico, lcuser_uq: "%" });
...
export async function PUT(req: NextRequest, { params }: P) {
    ...
    try {
        ...
        return NextResponse.json({ success: true, message: "Sales rep updated." });
    } catch (err: any) {
...
export async function DELETE(_req: NextRequest, { params }: P) {
    const { unico } = await params;
    try {
        const r = await executeProcedure("sp_flower_salesmen_delete", { lcsalesman_uq: unico });
        const row = r.recordset?.[0];
        if (row?.error === 1 || row?.Error === 1) return NextResponse.json({ success: false, error: row.message || row.Message }, { status: 400 });
