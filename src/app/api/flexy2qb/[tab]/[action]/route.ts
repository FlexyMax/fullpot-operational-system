...
export async function POST(req: NextRequest, context: { params: Promise<{ tab: string; action: string }> }) {
  try {
    ...
    switch (tab) {
      ...
      case "purchases":
        if (action === "years") procName = "sp_NC_AWB_years";
        ...
        } else if (action === "ready") procName = "sp_flower_packing_box_ready_to_qbooks";
        else if (action === "sent") {
        ...
        } else if (action === "tpo") procName = "sp_NC_Vendor_Bills_Ready2Qbooks";
        break;
      ...
      case "ocharges":
        if (action === "years") procName = "sp_NC_AWB_years";
          ...
          spParams = { ldawb_date: body.ldawb_date };
        } else if (action === "ready") procName = "sp_flower_awb_charges_ready_to_qbooks";
        else if (action === "sent") {
      ...
      case "purchases-credits":
        ...
        } else if (action === "ready") procName = "sp_flower_accounts_pay_cr_qb_ready_to_qbooks";
        else if (action === "sent") {
      ...
      case "sales":
        if (action === "years") procName = "sp_NC_flower_invoice_years";
        else if (action === "dates") {
        ...
        } else if (action === "ready") procName = "sp_flower_invoice_ready_to_qbooks";
        else if (action === "sent") {
      ...
      case "sales-costs":
        if (action === "years") procName = "sp_NC_flower_invoice_years";
          ...
          spParams = { ldinvoice_date: body.ldinvoice_date };
        } else if (action === "ready") procName = "sp_NC_invoice_costs_ready_to_qbooks";
        else if (action === "sent") {
      ...
      case "sales-credits":
        if (action === "years") procName = "sp_NC_flower_invoice_years";
          ...
          spParams = { ldcr_date: body.ldcr_date };
        } else if (action === "ready") procName = "sp_flower_invoice_box_crdb_qb_ready";
        else if (action === "sent") {
      ...
      case "payments":
        if (action === "dates") procName = "sp_flower_accounts_income_dates";
        else if (action === "not-ready") {
          procName = "sp_flower_accounts_income_dates_qb_list";
          spParams = { ldin_date: body.ldin_date };
        } else if (action === "ready") procName = "sp_flower_accounts_income_dates_qb_ready";
        else if (action === "sent") {
    ...
    // Check if the SP returned an error object like { error: true, message: '...' }
    if (result.recordset && result.recordset[0] && (result.recordset[0].error === true || result.recordset[0].error === 1)) {
