                ...
                <div className="flex flex-col flex-1 overflow-hidden p-1.5 gap-1.5">
                    ...
                    <PanelGrid
                        ...
                        menuItems={[
                            ...
                            { label: "Email", icon: Mail, color: "gray", onClick: async ()=>{ if(!selInvoice||!selCustomer) return; const email = t(selCustomer.ap_email||selCustomer.email); if(!email){ toast.error("Customer has no email on file."); return; } const invUq = selInvoice.invoice_uq || selInvoice.unico; toast.info("Sending invoice..."); try{ const r=await fetch("/api/customer-payments/invoice-email",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({invoice_uq:invUq,email,customer_name:t(selCustomer.customer),invoice_no:t(selInvoice.invoice_no)})}); const d=await r.json(); d.success?toast.success("Invoice emailed."):toast.error(d.error||"Failed to send."); }catch(e:any){toast.error(e.message);} }, disabled: !selInvoice||!perms.canReport },
                            { label: "Invoice", icon: Printer, color: "gray", onClick: ()=>{ if(!selInvoice) return; const invUq = selInvoice.invoice_uq || selInvoice.unico; setInvoiceReportUrl(`/api/customer-payments/invoice-print/${invUq}`); }, disabled: !selInvoice||!perms.canReport },
