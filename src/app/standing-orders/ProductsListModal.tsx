...
export function ProductsListModal({ soUnico, cases, onClose, onAdded }: Props) {
    ...
    const doSearch = (q: string) => {
        ...
        const desc = q.trim() ? `%${q.trim()}%` : "%";
        timerRef.current = setTimeout(async () => {
            setLoading(true);
            try {
                const r = await fetch(`/api/standing-orders/products?q=${encodeURIComponent(q.trim() || " ")}&limit=200`);
                const j = await r.json();
                setProducts(Array.isArray(j) ? j.map((p: any) => ({
                    unico:       t(p.unico        ?? p.UNICO),
                    description: t(p.description  ?? p.DESCRIPTION),
                    short:       t(p.short         ?? p.SHORT),
        ...
        try {
            const r = await fetch("/api/standing-orders/line", {
                ...
                body: JSON.stringify({
                    ...
                    active:     true,
                    details: "", pccode: "", upc: selected.upc,
                    food: false, food_uq: "",
                    cporder_no: "", upc_text: "", retail_price: 0,
                    boxcode2: "", color_breakdown: "", upc_notes: "", additional_notes: "",
                }),
            });
            const j = await r.json();
            if (!r.ok || !j.success) throw new Error(j.error || "Failed");
            toast.success(`"${selected.description.slice(0, 40)}" added to order`);
            onAdded();
            setSelected(null);
        } catch (e: any) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-5xl max-h-[92vh] flex flex-col overflow-hidden">
                ...
                <div className="h-10 bg-[#374151] flex items-center justify-between px-4 shrink-0 rounded-t-lg">
                    <span className="font-black text-[11px] text-white uppercase tracking-widest">Products List</span>
                ...
                <div className="px-3 py-2 border-b border-gray-200 flex items-center gap-2 shrink-0">
                    ...
                    <span className="text-[10px] text-gray-400 shrink-0">{products.length} products</span>
                </div>
                    ...
                    <table className="min-w-full text-[11px] text-left">
                        <thead className="sticky top-0 z-10">
                            <tr className="bg-gray-100 text-gray-700">
                                ...
                                <th className="px-2 py-1.5 font-bold border-b border-gray-200">BoxCode</th>
                                <th className="px-2 py-1.5 font-bold border-b border-gray-200">UPC</th>
                                <th className="px-2 py-1.5 font-bold border-b border-gray-200 max-w-[100px]">Customer</th>
                            </tr>
                        </thead>
                        <tbody>
                            {products.map((p, i) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <td className="px-2 py-1 text-[10px] text-gray-500">{p.boxcode}</td>
                                        <td className="px-2 py-1 text-[10px] text-gray-500">{p.upc}</td>
                                        <td className="px-2 py-1 text-[10px] text-gray-400 max-w-[100px] truncate">{p.customer}</td>
                                    </tr>
                                );
                            })}
                    ...
                    <div className="border-t-2 border-[#FB7506] bg-orange-50 px-4 py-3 shrink-0">
                        <div className="flex items-start gap-3 flex-wrap">
                            <div className="flex-1 min-w-0">
                                <p className="text-[10px] font-black text-[#FB7506] uppercase tracking-widest mb-1">Selected</p>
                            ...
                            <div className="flex items-end gap-2 flex-wrap shrink-0">
                                <div>
                                    <label className="block text-[9px] font-bold text-gray-500 uppercase tracking-wide mb-0.5">Case</label>
                                    <select value={addForm.case_uq} onChange={e => af("case_uq", e.target.value)}
                                        className={`${inp} w-36`}>
                                ...
                                <div>
                                    <label className="block text-[9px] font-bold text-gray-500 uppercase tracking-wide mb-0.5">Qty</label>
                                    <input type="number" value={addForm.qty} onChange={e => af("qty", e.target.value)}
                                        className={`${inp} w-16`} min="1" />
                                </div>
                                <div>
                                    <label className="block text-[9px] font-bold text-gray-500 uppercase tracking-wide mb-0.5">Bx/Case</label>
                                    <input type="number" value={addForm.packs_box} onChange={e => af("packs_box", e.target.value)}
                                        className={`${inp} w-16`} min="1" />
                                </div>
                                <div>
                                    <label className="block text-[9px] font-bold text-gray-500 uppercase tracking-wide mb-0.5">Un/Bch</label>
                                    <input type="number" value={addForm.units_pack} onChange={e => af("units_pack", e.target.value)}
                                        className={`${inp} w-16`} min="1" />
                                </div>
                                <div>
                                    <label className="block text-[9px] font-bold text-gray-500 uppercase tracking-wide mb-0.5">Price</label>
                                    <input type="number" step="0.01" value={addForm.price} onChange={e => af("price", e.target.value)}
                                        className={`${inp} w-20`} />
                                </div>
