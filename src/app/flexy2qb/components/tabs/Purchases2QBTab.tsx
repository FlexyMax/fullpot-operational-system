...
export default function Purchases2QBTab() {
    ...
    const { data: years = EMPTY_ARR } = useQuery({
        queryKey: ["f2qb-purch-years"],
        queryFn: async () => (await (await fetch("/api/flexy2qb/purchases/years", { method: "POST", body: "{}" })).json()).data || [],
    });
    const { data: dates = EMPTY_ARR, isFetching: loadingDates } = useQuery({
        queryKey: ["f2qb-purch-dates", selectedYear],
        queryFn: async () => (await (await fetch("/api/flexy2qb/purchases/dates", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ lnYear: selectedYear }) })).json()).data || [],
        enabled: !!selectedYear,
    });
    const { data: notReady = EMPTY_ARR, isFetching: loadingNR } = useQuery({
        queryKey: ["f2qb-purch-nr", selectedDate, refreshTrigger],
        queryFn: async () => (await (await fetch("/api/flexy2qb/purchases/not-ready", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ldawb_date: selectedDate, pageNo: 1, pageSize: 500 }) })).json()).data || [],
        enabled: !!selectedDate && subTab === "not-ready",
    });
    const { data: readyData = EMPTY_ARR, isFetching: loadingReady } = useQuery({
        queryKey: ["f2qb-purch-ready", refreshTrigger],
        queryFn: async () => (await (await fetch("/api/flexy2qb/purchases/ready", { method: "POST", body: "{}" })).json()).data || [],
        enabled: subTab === "ready",
    });
    const { data: sentData = EMPTY_ARR, isFetching: loadingSent } = useQuery({
        queryKey: ["f2qb-purch-sent", selectedDate, refreshTrigger],
        queryFn: async () => (await (await fetch("/api/flexy2qb/purchases/sent", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ldawb_date: selectedDate }) })).json()).data || [],
        enabled: !!selectedDate && subTab === "sent",
    });
    const { data: tpoData = EMPTY_ARR } = useQuery({
        queryKey: ["f2qb-purch-tpo", refreshTrigger],
        queryFn: async () => (await (await fetch("/api/flexy2qb/purchases/tpo", { method: "POST", body: "{}" })).json()).data || [],
        enabled: subTab === "ready",
    });
    ...
    const mobileItems = [
        ...
        { grid: "ready", label: "Not Ready", icon: X, color: "red", onClick: () => { if (selReady === undefined) return toast.error("Select a row"); markReady.mutate({ lcpacking_box: readyData[selReady!]?.packing_uq, lcawbcode: readyData[selReady!]?.awbcode, llready: false }); }, disabled: !canWrite || selReady === undefined },
        { grid: "ready", label: "By Date", icon: Calendar, color: "red", onClick: () => { if (!selectedDate) return toast.error("Select a date"); markReadyByDate.mutate({ ldAwbDate: selectedDate, llready: false }); }, disabled: !canWrite || !selectedDate },
                    ...
                    <div className="overflow-y-auto flex-1">
                        <table className="min-w-full text-xs text-left">
                            <thead className="bg-gray-100 border-b text-gray-700 font-bold sticky top-0">
                                <tr><th className="p-2">AWB Date</th><th className="p-2 text-right">Bills</th></tr>
                            </thead>
                            <tbody>
                                {dates.map((d: any, i: number) => {
                                    const ds = d.awbdate || d.awb_date;
                                    const active = selectedDate === ds;
                                    return <tr key={i} onClick={() => setSelectedDate(ds)} className={cn("border-b cursor-pointer transition-colors", active ? "!bg-blue-100 ring-2 ring-inset ring-blue-300" : "hover:bg-blue-50")}><td className="p-2 font-medium">{new Date(ds).toLocaleDateString("en-US")}</td><td className="p-2 text-right text-gray-500">{d.AWBills || d.records || 0}</td></tr>;
                                })}
                            </tbody>
                        </table>
                    </div>
                    ...
                    {subTab === "not-ready" && (
                        <div className="flex flex-col h-full bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                            <div className="hidden md:block">
                                <TopActionBar disabled={!canWrite} title="Data in Flexymax Not Ready To Transfer To QBooks" actions={[
                                    { label: "Ready By Invoice", colorClass: "text-green-600", onClick: () => { if (selNR === undefined || !notReady[selNR]) return toast.error("Select a row first"); markReadyInvoice.mutate({ lcpacking_uq: notReady[selNR].pack_uq, llready: true }); } },
                                    { label: "Ready By Date", colorClass: "text-green-600", onClick: () => { if (!selectedDate) return toast.error("Select a date first"); markReadyByDate.mutate({ ldAwbDate: selectedDate, llready: true }); } },
                                ]} />
                            </div>
                            ...
                            <TabTable showToolbar loading={loadingNR} rows={notReady} selectedIdx={selNR}
                                ...
                                empty={selectedDate ? "No pending data" : "Select a date"} columns={[{ key: "awbcode", label: "AWB Code" }, { key: "grower", label: "Grower" }, { key: "invoice_no", label: "Invoice" }, { key: "total_units", label: "Units", className: "text-right" }, { key: "total_cost", label: "Cost", className: "text-right font-semibold" }]} />
                        </div>
                    )}
                    {subTab === "ready" && (
                        <div className="flex flex-col h-full bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                            <div className="hidden md:block">
                                <TopActionBar disabled={!canWrite} title="Data in Flexymax Ready To QBooks" actions={[
                                    { label: "Invoice Mark as Not Ready", colorClass: "text-red-500", onClick: () => { if (selReady === undefined || !readyData[selReady]) return toast.error("Select a row first"); markReady.mutate({ lcpacking_box: readyData[selReady].packing_uq, lcawbcode: readyData[selReady].awbcode, llready: false }); } },
                                    { label: "Mark as Not Ready By Date", colorClass: "text-red-500", onClick: () => { if (!selectedDate) return toast.error("Select a date first"); markReadyByDate.mutate({ ldAwbDate: selectedDate, llready: false }); } },
                                    { label: "Sent By Invoice", colorClass: "text-green-600", onClick: () => { if (selReady === undefined || !readyData[selReady]) return toast.error("Select a row first"); sendToQb.mutate({ lcawbcode_aux: readyData[selReady].awbcode, llready: true, llByReadyByDate: false }); } },
                                    { label: "Sent By Date", colorClass: "text-green-600", onClick: () => { if (!selectedDate) return toast.error("Select a date first"); sendToQb.mutate({ lcawbcode_aux: null, llready: true, llByReadyByDate: true }); } },
                                ]} />
                            </div>
                            ...
                            <TabTable showToolbar onDownload={downloadCSV} loading={loadingReady} rows={readyData} selectedIdx={selReady}
                                onSelectIdx={i => { const s = selReady === i; setSelReady(s ? undefined : i); setActiveGrid(s ? null : "ready"); }}
                                empty="No data ready" columns={[{ key: "awbcode", label: "AWB Code" }, { key: "Vendor", label: "Vendor" }, { key: "Bill_No", label: "Bill No" }, { key: "Amount", label: "Amount", className: "text-right font-semibold" }]} />
                        </div>
                    )}
                    {subTab === "sent" && (
                        <div className="flex flex-col h-full bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                            <div className="hidden md:block">
                                <TopActionBar disabled={!canWrite} title="Flexymax Data Sent To QBooks" actions={[
                                    { label: "Mark as Not Sent", colorClass: "text-red-500", onClick: () => { if (selSent === undefined || !sentData[selSent]) return toast.error("Select a row first"); sendToQb.mutate({ lcawbcode_aux: sentData[selSent].awbcode, llready: false, llByReadyByDate: false }); } },
                                    { label: "Mark as Not Sent By Date", colorClass: "text-red-500", onClick: () => { if (!selectedDate) return toast.error("Select a date first"); sendToQb.mutate({ lcawbcode_aux: null, llready: false, llByReadyByDate: true }); } },
                                ]} />
                            </div>
                            ...
                            <TabTable showToolbar loading={loadingSent} rows={sentData} selectedIdx={selSent}
                                ...
                                empty={selectedDate ? "No data sent" : "Select a date"} columns={[{ key: "awbcode", label: "AWB Code" }, { key: "grower", label: "Grower" }, { key: "invoice_no", label: "Invoice" }, { key: "total_cost", label: "Cost", className: "text-right font-semibold" }]} />
                        </div>
                    )}
                </div>
            </div>
