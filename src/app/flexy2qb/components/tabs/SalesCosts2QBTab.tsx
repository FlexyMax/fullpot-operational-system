...
export default function SalesCosts2QBTab() {
    ...
    const { data: years = EMPTY_ARR } = useQuery({
        queryKey: ["f2qb-sc-years"],
        queryFn: async () => (await (await fetch("/api/flexy2qb/sales-costs/years", { method: "POST", body: "{}" })).json()).data || [],
    });
    const { data: dates = EMPTY_ARR, isFetching: loadingDates } = useQuery({
        queryKey: ["f2qb-sc-dates", selectedYear],
        queryFn: async () => (await (await fetch("/api/flexy2qb/sales-costs/dates", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ lnYear: selectedYear }) })).json()).data || [],
        enabled: !!selectedYear,
    });
    const { data: notReady = EMPTY_ARR, isFetching: loadingNR } = useQuery({
        queryKey: ["f2qb-sc-nr", selectedDate, refreshTrigger],
        queryFn: async () => (await (await fetch("/api/flexy2qb/sales-costs/not-ready", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ldinvoice_date: selectedDate }) })).json()).data || [],
        enabled: !!selectedDate && subTab === "not-ready",
    });
    const { data: readyData = EMPTY_ARR, isFetching: loadingReady } = useQuery({
        queryKey: ["f2qb-sc-ready", refreshTrigger],
        queryFn: async () => (await (await fetch("/api/flexy2qb/sales-costs/ready", { method: "POST", body: "{}" })).json()).data || [],
        enabled: subTab === "ready",
    });
    const { data: sentData = EMPTY_ARR, isFetching: loadingSent } = useQuery({
        queryKey: ["f2qb-sc-sent", selectedDate, refreshTrigger],
        queryFn: async () => (await (await fetch("/api/flexy2qb/sales-costs/sent", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ldinvoice_date: selectedDate }) })).json()).data || [],
                        ...
                        <div className="flex flex-col h-full bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                            <div className="hidden md:block">
                                <TopActionBar disabled={!canWrite} title="Data in Flexymax Not Ready" actions={[
                                    ...
                                    { label: "Ready By Date", colorClass: "text-green-600", onClick: () => { if (!selectedDate) return toast.error("Select a date first"); markReadyByDate.mutate({ ldInvoice_date: selectedDate, llready: true }); } },
                                ]} />
                            </div>
                        ...
                        <div className="flex flex-col h-full bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                            <div className="hidden md:block">
                                <TopActionBar disabled={!canWrite} title="Data Ready To QBooks" actions={[
                                    ...
                                    { label: "Mark as Not Ready By Date", colorClass: "text-red-500", onClick: () => { if (!selectedDate) return toast.error("Select a date first"); markReadyByDate.mutate({ ldInvoice_date: selectedDate, llready: false }); } },
