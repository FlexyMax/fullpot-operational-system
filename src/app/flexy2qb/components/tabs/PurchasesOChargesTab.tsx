...
export default function PurchasesOChargesTab() {
    ...
    const { data: years = EMPTY_ARR } = useQuery({
        queryKey: ["f2qb-och-years"],
        queryFn: async () => (await (await fetch("/api/flexy2qb/ocharges/years", { method: "POST", body: "{}" })).json()).data || [],
    });
    const { data: dates = EMPTY_ARR, isFetching: loadingDates } = useQuery({
        queryKey: ["f2qb-och-dates", selectedYear],
        queryFn: async () => (await (await fetch("/api/flexy2qb/ocharges/dates", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ lnYears: selectedYear }) })).json()).data || [],
        enabled: !!selectedYear,
    });
    const { data: notReady = EMPTY_ARR, isFetching: loadingNR } = useQuery({
        queryKey: ["f2qb-och-nr", selectedDate, refreshTrigger],
        queryFn: async () => (await (await fetch("/api/flexy2qb/ocharges/not-ready", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ldawb_date: selectedDate }) })).json()).data || [],
        enabled: !!selectedDate && subTab === "not-ready",
    });
    const { data: readyData = EMPTY_ARR, isFetching: loadingReady } = useQuery({
        queryKey: ["f2qb-och-ready", refreshTrigger],
        queryFn: async () => (await (await fetch("/api/flexy2qb/ocharges/ready", { method: "POST", body: "{}" })).json()).data || [],
        enabled: subTab === "ready",
    });
    const { data: sentData = EMPTY_ARR, isFetching: loadingSent } = useQuery({
        queryKey: ["f2qb-och-sent", selectedDate, refreshTrigger],
        queryFn: async () => (await (await fetch("/api/flexy2qb/ocharges/sent", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ldawb_date: selectedDate }) })).json()).data || [],
                    ...
                    <div className="overflow-y-auto flex-1">
                        <table className="min-w-full text-xs text-left">
                            <thead className="bg-gray-100 border-b text-gray-700 font-bold sticky top-0">
                                <tr><th className="p-2">AWB Date</th><th className="p-2 text-right">Charges</th></tr>
                            </thead>
                            <tbody>
                                {dates.map((d: any, i: number) => {
                                    const ds = d.awbdate || d.awb_date;
                                    const active = selectedDate === ds;
                                    return <tr key={i} onClick={() => setSelectedDate(ds)} className={cn("border-b cursor-pointer transition-colors", active ? "!bg-blue-100 ring-2 ring-inset ring-blue-300" : "hover:bg-blue-50")}><td className="p-2 font-medium">{new Date(ds).toLocaleDateString("en-US")}</td><td className="p-2 text-right text-gray-500">{d.AWBCharges || 0}</td></tr>;
                                })}
                            </tbody>
                        </table>
                    </div>
                    ...
                    {subTab === "not-ready" && (
                        <div className="flex flex-col h-full bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                            <div className="hidden md:block">
                                <TopActionBar disabled={!canWrite} title="Data in Flexymax Not Ready To Transfer" actions={[
                                    { label: "Mark Ready", colorClass: "text-green-600", onClick: () => { if (selNR === undefined || !notReady[selNR]) return toast.error("Select a row first"); markReady.mutate({ lcCharge_uq: notReady[selNR].unico, llready: true, llUpdateByDate: false }); } },
                                ]} />
                            </div>
                            ...
                            <TabTable showToolbar loading={loadingNR} rows={notReady} selectedIdx={selNR}
                                ...
                                empty={selectedDate ? "No pending data" : "Select a date"} columns={[{ key: "awbcode", label: "AWB Code" }, { key: "grower", label: "Grower" }, { key: "Bill_No", label: "Bill No" }, { key: "total_invoice", label: "Total", className: "text-right font-semibold" }]} />
                        </div>
                    )}
                    {subTab === "ready" && (
                        <div className="flex flex-col h-full bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                            <div className="hidden md:block">
                                <TopActionBar disabled={!canWrite} title="Data Ready To QBooks" actions={[
                                    { label: "Send to QB", colorClass: "text-blue-600", onClick: () => { if (selReady === undefined || !readyData[selReady]) return toast.error("Select a row first"); sendToQb.mutate({ lcCharge_uq: readyData[selReady].unico, llready: true, llByReadyByDate: false }); } },
                                    { label: "Unmark Ready", colorClass: "text-red-500", onClick: () => { if (selReady === undefined || !readyData[selReady]) return toast.error("Select a row first"); markReady.mutate({ lcCharge_uq: readyData[selReady].unico, llready: false, llUpdateByDate: false }); } },
                                ]} />
                            </div>
                    ...
                    {subTab === "sent" && (
                        <div className="flex flex-col h-full bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                            ...
                            <TabTable showToolbar loading={loadingSent} rows={sentData} selectedIdx={selSent}
                                ...
                                empty={selectedDate ? "No data sent" : "Select a date"} columns={[{ key: "awbcode", label: "AWB Code" }, { key: "grower", label: "Grower" }, { key: "Bill_No", label: "Bill No" }, { key: "total_invoice", label: "Total", className: "text-right font-semibold" }]} />
                        </div>
                    )}
                </div>
            </div>
