...
export default function PurchasesCreditsTab() {
    ...
    const { data: years = EMPTY_ARR } = useQuery({ queryKey: ["f2qb-pcr-years"], queryFn: async () => (await (await fetch("/api/flexy2qb/purchases-credits/years", { method: "POST", body: "{}" })).json()).data || [] });
    const { data: dates = EMPTY_ARR, isFetching: loadingDates } = useQuery({ queryKey: ["f2qb-pcr-dates", selectedYear], queryFn: async () => (await (await fetch("/api/flexy2qb/purchases-credits/dates", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ lnYears: selectedYear }) })).json()).data || [], enabled: !!selectedYear });
    const { data: notReady = EMPTY_ARR, isFetching: loadingNR } = useQuery({ queryKey: ["f2qb-pcr-nr", selectedDate, refreshTrigger], queryFn: async () => (await (await fetch("/api/flexy2qb/purchases-credits/not-ready", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ldcr_date: selectedDate }) })).json()).data || [], enabled: !!selectedDate && subTab === "not-ready" });
    const { data: readyData = EMPTY_ARR, isFetching: loadingReady } = useQuery({ queryKey: ["f2qb-pcr-ready", refreshTrigger], queryFn: async () => (await (await fetch("/api/flexy2qb/purchases-credits/ready", { method: "POST", body: "{}" })).json()).data || [], enabled: subTab === "ready" });
    const { data: sentData = EMPTY_ARR, isFetching: loadingSent } = useQuery({ queryKey: ["f2qb-pcr-sent", selectedDate, refreshTrigger], queryFn: async () => (await (await fetch("/api/flexy2qb/purchases-credits/sent", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ldcr_date: selectedDate }) })).json()).data || [], enabled: !!selectedDate && subTab === "sent" });
    ...
    const markReadyByDate = useMutation({ mutationFn: async (p: any) => (await (await fetch("/api/flexy2qb/purchases-credits/update-ready-date", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(p) })).json()), onSuccess: onMutate });
    ...
    const mobileItems = [
        ...
        { grid: "not-ready", label: "By Date",   icon: Calendar,   color: "green", onClick: () => { if (!selectedDate) return toast.error("Select a date"); markReadyByDate.mutate({ lccr_uq: null, llready: true, ldcr_date: selectedDate }); },                                             disabled: !canWrite || !selectedDate },
        ...
        { grid: "ready",     label: "Send Date", icon: Calendar,   color: "blue",  onClick: () => { if (!selectedDate) return toast.error("Select a date"); sendToQb.mutate({ lccr_uq: null, llready: true, llSendByDate: true, ldcr_date: selectedDate }); },                                 disabled: !canWrite || !selectedDate },
        ...
        { grid: "sent",      label: "By Date",   icon: Calendar,   color: "red",   onClick: () => { if (!selectedDate) return toast.error("Select a date"); sendToQb.mutate({ lccr_uq: null, llready: false, llSendByDate: true, ldcr_date: selectedDate }); },                               disabled: !canWrite || !selectedDate },
    ];
    ...
    return (
        <div className="flex flex-col md:flex-row h-full gap-2">
            <div className="md:hidden mb-1">
                <MobileDateCalendar
                    dates={dates}
                    dateGetter={row => row.cd_date}
                    countGetter={row => row.Credits || 0}
                    ...
                    {subTab === "not-ready" && (
                        <PanelGrid title="Not Ready" icon={Clock} recordCount={fNR.length} refreshing={loadingNR}
                            ...
                            menuItems={[
                                ...
                                { label: "Ready By Date",   icon: Calendar, color: "green", onClick: () => { if (!selectedDate) return toast.error("Select a date first"); markReadyByDate.mutate({ lccr_uq: null, llready: true, ldcr_date: selectedDate }); }, disabled: !canWrite || !selectedDate },
                            ]}
                    ...
                    {subTab === "ready" && (
                        <PanelGrid title="Ready to QB" icon={CheckCircle2} recordCount={fReady.length} refreshing={loadingReady}
                            ...
                            menuItems={[
                                ...
                                { separator: true },
                                { label: "Send By Credit", icon: ArrowRight, color: "blue", onClick: () => { if (selReady === undefined || !readyData[selReady]) return toast.error("Select a row first"); sendToQb.mutate({ lccr_uq: readyData[selReady].unico, llready: true, llSendByDate: false }); }, disabled: !canWrite || selReady === undefined },
                                { label: "Send By Date",   icon: Calendar,   color: "blue", onClick: () => { if (!selectedDate) return toast.error("Select a date first"); sendToQb.mutate({ lccr_uq: null, llready: true, llSendByDate: true, ldcr_date: selectedDate }); }, disabled: !canWrite || !selectedDate },
                            ]}
                            className="h-full flex flex-col">
                            <PanelGridTable>
                                <PanelGridThead><PanelGridTh>Invoice</PanelGridTh><PanelGridTh>Grower</PanelGridTh><PanelGridTh align="right">Amount</PanelGridTh></PanelGridThead>
                    ...
                    {subTab === "sent" && (
                        <PanelGrid title="Sent to QB" icon={CheckCheck} recordCount={fSent.length} refreshing={loadingSent}
                            ...
                            menuItems={[
                                ...
                                { label: "Not Sent By Date",   icon: Calendar,  color: "red", onClick: () => { if (!selectedDate) return toast.error("Select a date first"); sendToQb.mutate({ lccr_uq: null, llready: false, llSendByDate: true, ldcr_date: selectedDate }); }, disabled: !canWrite || !selectedDate },
                            ]}
                            className="h-full flex flex-col">
                            <PanelGridTable>
                                <PanelGridThead><PanelGridTh>Invoice</PanelGridTh><PanelGridTh>Grower</PanelGridTh><PanelGridTh align="right">Amount</PanelGridTh></PanelGridThead>
