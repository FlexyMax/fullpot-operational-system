...
function ConfirmDlg({ title, msg, onConfirm, onCancel, saving }: any) {
    return (
...
export default function CarriersDefinitionPage() {
    ...
    const {
        carrSearch, setCarrSearch, selUnico, setSelUnico,
    ...
    const [invDateIni, setInvDateIni] = useState(() => {
        const d = new Date(); d.setDate(d.getDate() - 30);
        return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
    });
    const [invDateEnd, setInvDateEnd] = useState(() => {
        const d = new Date();
        return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
    });
    ...
    const {
        ...
        queryKey: ["carr-list", carrSearch],
        queryFn: ({ pageParam = 1 }) =>
            sF(`/api/masters/carriers?page=${pageParam}&size=${PAGE_SIZE}&search=${encodeURIComponent(carrSearch)}`),
        getNextPageParam: (lastPage: any, allPages: any[]) => {
            const loaded = allPages.flatMap((p: any) => p.rows ?? []).length;
            return loaded < (lastPage.total ?? 0) ? allPages.length + 1 : undefined;
        },
        initialPageParam: 1,
    });

    const flatRows:   any[]  = carriersData?.pages?.flatMap((p: any) => p.rows ?? []) ?? [];
    const totalCount: number = (carriersData?.pages?.[0] as any)?.total ?? 0;
    ...
    /* ── Auto-load first carrier ─────────────────────────────────────────── */
    useEffect(() => {
        if (flatRows.length > 0 && !selUnico && !autoLoaded.current) {
            autoLoaded.current = true;
            fetchCarrierDetail(flatRows[0].unico);
            setSelUnico(flatRows[0].unico);
        }
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [flatRows.length]);

    /* ── Detail fetch ────────────────────────────────────────────────────── */
    const fetchCarrierDetail = async (unico: string) => {
        try {
            const d = await sF(`/api/masters/carriers/${unico}`);
            setForm({
                unico:             t(d.unico),
                active:            Boolean(d.active),
                carrier:           t(d.carrier),
                carriercode:       t(d.carriercode ?? d.code),
                contact:           t(d.contact),
                address:           t(d.address),
                city:              t(d.city),
                state:             t(d.state),
    ...
    const loadCarrierDetail = async (unico: string) => {
        if (selUnico === unico) { setSelUnico(null); return; }
        setSelUnico(unico);
        await fetchCarrierDetail(unico);
    };
    ...
    const handleSave = async () => {
        ...
        try {
            if (mode === "add") {
                ...
                const res  = await fetch(`/api/masters/carriers/${selUnico}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
                const data = await res.json();
                if (!data.success) throw new Error(data.error);
                logAction("Edit", selUnico!);
    ...
    const handleOthersSave = async () => {
        ...
        try {
            ...
            await fetchCarrierDetail(selUnico);
        } catch (e: any) {
            toast.error(e.message);
        } finally {
            setSaving(false);
        ...
        <div className="flex flex-col h-[100dvh] bg-[#f4f6f8] overflow-hidden font-sans text-[#333]">
            ...
            <div className="bg-[#F5F3F3] border border-[#DBD9D9] rounded-lg mx-2 mt-2 px-3 py-2 flex items-center gap-2 shrink-0 shadow-sm flex-wrap">
                <div className="relative flex-1 md:flex-none">
                    ...
                    <input type="text" value={carrSearch} onChange={e => { setCarrSearch(e.target.value); autoLoaded.current = false; }}
            ...
            <div className={cn("flex flex-1 min-h-0 px-2 pt-2", selUnico ? "pb-[76px] md:pb-2" : "pb-2")}>
                <PanelGrid
                    ...
                    onRefresh={() => { autoLoaded.current = false; refetchList(); }}
                ...
                >
                    <PanelGridTable>
                        ...
                        <PanelGridTbody>
                            {flatRows.length === 0 && !loadingList ? (
                                <PanelGridTr className="pointer-events-none">
                                    <PanelGridTd colSpan={8} align="center" className="py-10 text-gray-300 italic">No carriers found</PanelGridTd>
                                </PanelGridTr>
                            ) : flatRows.map((c: any) => (
                                <PanelGridTr key={c.unico} selected={c.unico === selUnico} onClick={() => loadCarrierDetail(c.unico)}>
                                    <PanelGridTd align="center">
                                        <div className={cn("w-2 h-2 rounded-full mx-auto", c.active ? "bg-green-400" : "bg-gray-300")} />
                                    </PanelGridTd>
                                    <PanelGridTd className="font-mono font-bold text-[#FB7506]">{t(c.code ?? c.carriercode)}</PanelGridTd>
                            ...
                            {/* Infinite scroll sentinel */}
                            {hasNextPage && (
                                <tr ref={sentinelRef} className="pointer-events-none">
                                    <td colSpan={8} className="py-2 text-center">
                                        {isFetchingNextPage && <RefreshCcw size={14} className="text-gray-400 animate-spin mx-auto" />}
                                    </td>
                                </tr>
                            )}
                        </PanelGridTbody>
            ...
            <div className={cn(
                ...
                <div className="flex items-center gap-1 overflow-x-auto px-4 scrollbar-none">
                    ...
                    <button onClick={() => setSelUnico(null)}
                        ...
                        <PanelGrid
                            ...
                            headerRight={
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-gray-400 text-[9px] font-bold uppercase">to</span>
                ...
                <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-2 sm:p-4" onClick={() => setCustModal(false)}>
                    <div className="bg-white rounded-lg shadow-2xl w-full max-w-4xl max-h-[85vh] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
                        ...
                        <PanelGrid
                            ...
                            title="Customers"
                            icon={Users}
                            recordCount={(customers as any[]).length}
                            refreshing={loadingCust}
                        >
                            <PanelGridTable>
                                <PanelGridThead>
                                    <PanelGridTh>Customer</PanelGridTh>
