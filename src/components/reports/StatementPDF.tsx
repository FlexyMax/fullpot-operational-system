...
const s = StyleSheet.create({
    page:       { padding: "24 28 40 28", fontSize: 8, fontFamily: "Helvetica", color: "#1F2937", backgroundColor: "#FFF" },

    // ── Header band ─────────────────────────────────────────────────────────────
    headerBand: { backgroundColor: DARK, marginHorizontal: -28, marginTop: -24, paddingHorizontal: 28, paddingVertical: 12, marginBottom: 10, flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
    coName:     { fontSize: 15, fontWeight: 700, color: ACCENT, letterSpacing: 0.5 },
    coSub:      { fontSize: 7, color: "#9CA3AF", marginTop: 2 },
    titleBlock: { alignItems: "flex-end" },
    docTitle:   { fontSize: 13, fontWeight: 700, color: "#FFFFFF", textTransform: "uppercase", letterSpacing: 1 },
    docSub:     { fontSize: 7.5, color: "#9CA3AF", marginTop: 3 },
    ...
    agingAmt:   { fontSize: 8.5, fontWeight: 700, color: "#FFFFFF" },
    agingAmtHL: { fontSize: 8.5, fontWeight: 700, color: ACCENT },
...
export function StatementPDF({ company, rows, fromDate, toDate }: Props) {
    ...
    // Aging from first data row (repeated on all rows by SP)
    ...
    // Totals from all rows
    const totDebits  = rows.reduce((s, r) => s + (parseFloat(r.debits)  || 0), 0);
    const totCredits = rows.reduce((s, r) => s + (parseFloat(r.credits) || 0), 0);
            ...
            <Page size="LETTER" orientation="portrait" style={s.page}>
                ...
                <View style={s.infoRow}>
                    ...
                    <View style={[s.infoCard, s.infoAccent]}>
                        ...
                        <Text style={[s.infoValue, { fontSize: 13, color: balance > 0 ? RED : GREEN }]}>{fmtN(balance)}</Text>
                        <Text style={s.infoLine}>Unapplied: {fmtN(unapply)}</Text>
                    </View>
                </View>
                ...
                <View style={s.agingRow}>
                    {[
                        ...
                        { label: "Total Balance",  val: balance, highlight: true },
                    ].map(({ label, val, highlight }, i) => (
                        <View key={i} style={s.agingCell}>
                            <Text style={s.agingLabel}>{label}</Text>
                            <Text style={highlight ? s.agingAmtHL : s.agingAmt}>{fmtN(val)}</Text>
                        </View>
                    ))}
                </View>
                ...
                {rows.map((row, i) => {
                    ...
                    const inv  = parseInt(row.invoice_no) || 0;
                    return (
                        <View key={i} style={i % 2 === 0 ? s.tr : s.trAlt} wrap={false}>
                            <Text style={[s.td, flex(1.2)]}>{fmtD(row.date)}</Text>
                            <Text style={[s.td, flex(2.5), { color: DARK, fontWeight: 700 }]}>{c(row.type)}</Text>
                            <Text style={[s.td, flex(1.5), { color: BLUE }]}>{inv > 0 ? String(inv) : ""}</Text>
                ...
                {/* ── Totals row ── */}
                <View style={s.totRow}>
                    <Text style={[s.totLabel, flex(1.2 + 2.5)]}>TOTALS ({rows.length} transactions)</Text>
                    <Text style={[s.td, flex(1.5)]}></Text>
                    <Text style={[s.totAmt, flex(1.2)]}>{fmtN(totDebits)}</Text>
                    <Text style={[s.totAmt, flex(1.2), { color: GREEN }]}>{fmtN(totCredits)}</Text>
                    <Text style={[s.totAmt, flex(1.2)]}>{fmtN(balance)}</Text>
                </View>
